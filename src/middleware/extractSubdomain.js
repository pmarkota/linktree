...
module.exports = (req, res, next) => {
  ...
  if (host.includes("tamilfreelancer.rest")) {
    ...
    const userExists = Object.entries(userRegistry).some(
      ([_, data]) => data.subdomain === subdomain
    );
  ...
  if (process.env.NODE_ENV === "development") {
    if (host.includes("localhost") || host.includes("ngrok")) {
      ...
      const userEntry = Object.entries(userRegistry).find(
        ([_, data]) => data.subdomain.split("-")[0] === subdomain
      );
